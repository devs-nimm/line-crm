...
export default function LinkBaseUrlSetting() {
  ...
  return (
    <>
      <UrlSettingCard
        title={t('アフィリリンクドメイン（全アカウント共通）')}
        description={
          <>
            {t('アフィリエイト配布リンクに短縮ドメインを使う場合に設定。例:')}{' '}
      ...
      />
      <UrlSettingCard
        title={t('メッセージ内リンクの短縮ドメイン（全アカウント共通）')}
        description={
          <>
            {t('配信メッセージの自動短縮リンク（/t/…）に使うドメイン。例:')}{' '}
            <code className="bg-gray-100 px-1 rounded">https://go.example.com</code>
            {' '}→ {t('リンクは')} <code className="bg-gray-100 px-1 rounded">https://go.example.com/t/Ab3xY9k</code> {t('形式に。')}
            {t('そのドメインの')} <code className="bg-gray-100 px-1 rounded">/t/*</code> {t('をパスそのまま Worker へ転送する設定（Redirect Rule 等）が必要。詳細は wiki「Tracked Links」参照')}
          </>
        }
        placeholder={t('https://go.example.com（空欄で Worker URL を使用）')}
