...
手順を示してください。`),
  ...
  const loadReminders = useCallback(async () => {
    ...
    try {
      ...
      setError(t('リマインダーの読み込みに失敗しました。もう一度お試しください。'))
    } finally {
      setLoading(false)
    }
  }, [selectedAccountId])
  ...
  const handleCreate = async () => {
    if (!form.name.trim()) {
      setFormError(t('リマインダー名を入力してください'))
      return
    }
    setSaving(true)
    setFormError('')
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(t('このリマインダーを削除してもよいですか？'))) return
    try {
  ...
  const handleAddStep = async () => {
    ...
    if (!stepForm.messageContent.trim()) {
      setStepFormError(t('メッセージ内容を入力してください'))
      return
    }
    setStepSaving(true)
    setStepFormError('')
    try {
      ...
      setStepFormError(t('ステップの追加に失敗しました'))
    } finally {
      setStepSaving(false)
    }
  }
    ...
    <div>
      ...
      {showCreate && (
        <div className="mb-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h2 className="text-sm font-semibold text-gray-800 mb-4">{t('新規リマインダーを作成')}</h2>
          <div className="space-y-4 max-w-lg">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">{t('リマインダー名')} <span className="text-red-500">*</span></label>
              <input
                type="text"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                placeholder={t('例: セミナー参加リマインダー')}
                value={form.name}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          <p className="text-gray-500">{t('リマインダーがありません。「新規リマインダー」から作成してください。')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              ...
              >
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    >
                      {reminder.isActive ? t('有効') : t('無効')}
                    </span>
                  </div>
                  <div className="flex items-center gap-4 mt-3 text-xs text-gray-400">
                    <span>{t('作成日')}: {new Date(reminder.createdAt).toLocaleDateString('ja-JP')}</span>
                    <span className="flex items-center gap-1">
                      {isExpanded ? `▲ ${t('閉じる')}` : `▼ ${t('詳細')}`}
                    </span>
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-200 p-5">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-4">
                      ...
                      >
                        {reminder.isActive ? t('無効にする') : t('有効にする')}
                      </button>
                      <button
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="text-xs font-semibold text-gray-700">
                            {t('ステップ')} ({expandedData.steps.length}{t('件')})
                          </h4>
                          <button
                        ...
                        {expandedData.steps.length === 0 ? (
                          <p className="text-xs text-gray-400 py-4 text-center">{t('ステップがありません。「ステップ追加」から作成してください。')}</p>
                        ) : (
                        ...
                        {showStepForm && (
                          <div className="mt-4 bg-white border border-gray-200 rounded-lg p-4">
                            <h5 className="text-xs font-semibold text-gray-700 mb-3">{t('ステップを追加')}</h5>
                            <div className="space-y-3 max-w-lg">
                              <div>
                                <label className="block text-xs font-medium text-gray-600 mb-1">{t('オフセット (分)')}</label>
                                <input
                                  type="number"
                                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                                  placeholder={t('例: -60 (1時間前), +30 (30分後)')}
